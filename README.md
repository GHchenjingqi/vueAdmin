...
内部执行流程：`tsx app.ts` → 自动建库 → Umzug 迁移建表 → 种子数据 → Vite HMR + Express 服务。

> ⚠️ **Windows 注意**：若 `tsx`/`esbuild` 报 `The service was stopped`，说明 esbuild 二进制文件缺失。  
> **修复方法**（确保版本匹配）：
> ```powershell
> cd server
...
| `tsx`/`esbuild` 报 `The service was stopped` | esbuild 二进制文件缺失或版本不匹配 | `cd server && npm install @esbuild/win32-x64@0.28.1 --ignore-scripts && Copy-Item node_modules\@esbuild\win32-x64\esbuild.exe node_modules\tsx\node_modules\esbuild\bin\esbuild.exe -Force`；兜底：`npx tsc -p tsconfig.json` 后 `node dist/app.js`（无 Vite HMR） |
...
| `server/package.json` 脚本 | `dev` 为 `tsx app.ts`，`migrate*` 为 `tsx scripts/migrate.ts ...` |
...
| esbuild 二进制 | `server/node_modules/tsx/node_modules/esbuild/bin/esbuild.exe` 必须存在且版本与 `tsx` 的 esbuild 一致（当前 0.28.1）；缺失时 `npm install @esbuild/win32-x64@0.28.1 --ignore-scripts` 后复制 |
